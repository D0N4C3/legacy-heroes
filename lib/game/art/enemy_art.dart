...
class EnemyArt {
  ...
  /// Draws the creature. [hitFlash] (0–1) briefly tints it red-white on a
  /// landed hit; [defeatProgress] (0–1) shrinks and fades it out on death.
  /// Both are additive and default to 0 (no change from a plain draw).
  static void draw(
    Canvas canvas,
    EnemyType type,
    double t, {
      ...
      canvas.saveLayer(
          ...
          Paint()
            ..colorFilter = ColorFilter.mode(
                Color.fromRGBO(255, 255, 255, fade), BlendMode.dstIn));
      canvas.translate(0, defeatProgress * 26);
      canvas.scale(max(0.05, fade));
    }
    ...
    if (hitFlash > 0) {
      canvas.saveLayer(
          ...
          Paint()
            ..colorFilter = ColorFilter.mode(
                Color.fromRGBO(255, 90, 90, hitFlash.clamp(0.0, 1.0)),
                BlendMode.srcATop));
    }
    ...
    if (hitFlash > 0) canvas.restore();
    if (defeatProgress > 0) canvas.restore();
    canvas.restore();
  }

  static void _shadow(Canvas canvas, double w) {
    canvas.drawOval(
      Rect.fromCenter(center: const Offset(0, 2), width: w, height: 12),
  ...
  static void _goblin(Canvas canvas, double t) {
    ...
    const skin = Color(0xFF5E8C3A);
    const skinDark = Color(0xFF3F6125);

    // Legs.
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(-12, -16, 8, 16),
    ...
    // Loincloth.
    canvas.drawRect(
      const Rect.fromLTWH(-14, -18, 28, 8),
    ...
    canvas.drawCircle(
      const Offset(0, 4),
      11,
      Paint()..color = skinDark.withValues(alpha: 0.25),
    );
    // Nose + grin.
  ...
  static void _wolf(Canvas canvas, double t) {
    ...
    canvas.save();
    canvas.translate(0, bob);
    const fur = Color(0xFF6B6F78);
    const furDark = Color(0xFF474A52);
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(-26, -36, 50, 24),
  ...
  static void _skeleton(Canvas canvas, double t) {
    ...
    canvas.save();
    canvas.translate(0, bob);
    const bone = Color(0xFFE8E4D4);
    const boneShade = Color(0xFFB9B3A0);
    ...
    // Shoulders + arms.
    canvas.drawRect(
      const Rect.fromLTWH(-14, -50, 28, 4),
      Paint()..color = bone,
    );
    canvas.drawRect(
      const Rect.fromLTWH(-15, -50, 4, 24),
      Paint()..color = bone,
    );
    canvas.save();
    ...
    // Rusty sword.
    canvas.drawRect(
      const Rect.fromLTWH(-2, -22, 4, 24),
      Paint()..color = const Color(0xFF8A7B5A),
    );
    canvas.restore();
    // Skull.
    canvas.save();
  ...
  static void _demon(Canvas canvas, double t, bool boss) {
    ...
    const body = Color(0xFF6E1B22);
    const bodyDark = Color(0xFF3E0E12);
    ...
    // Legs + body.
    canvas.drawRect(
      const Rect.fromLTWH(-12, -18, 9, 18),
      Paint()..color = bodyDark,
    );
    canvas.drawRect(
      const Rect.fromLTWH(3, -18, 9, 18),
      Paint()..color = bodyDark,
    );
    final torso = Path()
      ..moveTo(-20, -16)
    ...
    canvas.drawPath(
      ...
      Paint()
        ..shader = const LinearGradient(
          ...
          colors: [Color(0xFF8E2630), Color(0xFF4E1018)],
        ).createShader(const Rect.fromLTWH(-26, -56, 52, 56)),
    );
    // Abs detail.
    canvas.drawLine(
      const Offset(0, -50),
    ...
    // Mouth.
    canvas.drawArc(
      Rect.fromCenter(center: const Offset(0, 6), width: 16, height: 10),
      0,
      pi,
      false,
      Paint()..color = const Color(0xFF2A0608),
    );
    _eyes(
      canvas,
      const Offset(-6, 0),
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..color = const Color(
          0xFFFF3A1A,
        ).withValues(alpha: 0.12 + 0.05 * sin(t * 6))
